...
class FullyConnectedNet(object):
    ...
    def __init__(
        ...
        for i in range(1, self.num_layers - 1):
            self.params['W' + str(i + 1)] = np.random.normal(
                0, weight_scale, hidden_dims[i - 1] * hidden_dims[i]).reshape((hidden_dims[i - 1], hidden_dims[i]))
    ...
    def loss(self, X, y=None):
        ...
        for i in np.arange(1, self.num_layers, 1):
            ...
            # Optional batchnorm
            if self.normalization:
                temp_out, temp_cache = batchnorm_forward(temp_out, self.params["gamma" + str(i)],
